...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    const appointments = await supabaseRequest(`/rest/v1/agendapro_public_booking_requests?agenda_slug=eq.${encodeURIComponent(slug)}&select=id,agenda_slug,service_name,requested_date,requested_time,status,metadata,created_at&limit=500`, { method: 'GET' }).catch(() => []);
    const booked_slots = Array.isArray(appointments) ? appointments.map(item => ({ id: item.id, agenda_slug: item.agenda_slug, service_name: item.service_name, requested_date: item.requested_date, requested_time: item.requested_time, status: item.status, metadata: item.metadata || {}, durationMinutes: item.metadata?.durationMinutes, endTime: item.metadata?.endTime })) : [];
