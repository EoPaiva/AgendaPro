...
module.exports = async function handler(req,res){
 ...
 try{
  ...
  const requestedUnitName = cleanText(body.unitName || body.unit || body.unitId || '',160);
  ...
  const unitNameOf = item => cleanText(item?.unit || item?.unitName || item?.unit_name || item?.metadata?.unitName || item?.metadata?.unit || item?.raw_payload?.unitName || item?.raw_payload?.unit || '',160) || 'Unidade principal';
  const unitServices = requestedUnitName ? services.filter(item => unitNameOf(item) === requestedUnitName) : services;
  const servicePool = unitServices.length ? unitServices : services;
  const selectedService = servicePool.find(item => String(item.id || item.name) === String(serviceId || serviceName)) || servicePool.find(item => item.name === serviceName);
  if(!selectedService) return res.status(400).json({ok:false,message:'Serviço indisponível ou inativo nesta agenda.'});
  const resolvedUnitName = requestedUnitName || unitNameOf(selectedService);
  ...
  const unitTeam = team.filter(item => unitNameOf(item) === resolvedUnitName);
  const teamPool = unitTeam.length ? unitTeam : team;
  const selectedProfessional = teamPool.find(item => String(item.id || item.name) === String(professionalId || professionalName)) || teamPool.find(item => item.name === professionalName);
  ...
  if(requestedUnitName && unitNameOf(selectedProfessional) !== resolvedUnitName) return res.status(400).json({ok:false,message:'Este profissional não atende na unidade selecionada.'});
