...
module.exports = async function handler(req,res){
 ...
 try{
  ...
  const inserted = await supabaseRequest('/rest/v1/agendapro_public_booking_requests',{method:'POST',headers:{Prefer:'return=representation'},body:JSON.stringify({agenda_id:agenda.id,account_id:agenda.account_id||null,company_id:agenda.company_id||null,agenda_slug:slug,business_name:agenda.business_name,customer_name:name,customer_phone:phone,customer_email:email || null,service_name:serviceName || selectedService.name || null,requested_date:date,requested_time:time,status:'requested',notes,metadata:{source:'public_booking',serviceId:serviceId || selectedService.id || selectedService.name || null,durationMinutes,endTime,date,startTime:time}})});
  await logActivity({ accountId: agenda.account_id || null, companyId: agenda.company_id || null, action: 'public_booking_created', title: 'Agendamento recebido', description: `${name} solicitou ${serviceName || 'um atendimento'} para ${date} às ${time}.`, severity: 'success', metadata: { slug, serviceName, date, time, durationMinutes } });
