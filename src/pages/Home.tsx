...
type AgendaDraft = {
  ...
  services: Array<{ id?: string; name: string; duration: string; durationMinutes?: number | string; price: string | number; value?: string | number; description: string; category?: string; unit?: string; unitName?: string; unitId?: string; active?: boolean; highlight?: boolean; order?: number | string; professionalsText?: string; professionalNames?: string[]; internalNotes?: string }>;
  team: Array<{ id?: string; name: string; role: string; whatsapp: string; email?: string; specialty?: string; unit?: string; unitName?: string; unitId?: string; avatarUrl?: string; active?: boolean; servicesText?: string; serviceNames?: string[]; availabilityNotes?: string; scheduleConfig?: Partial<ScheduleConfig>; schedule?: Partial<ScheduleConfig>; availability?: Partial<ScheduleConfig> | Record<string, any> }>;
...
function PrivateAgendaDashboardPage({ route }: { route: string }) {
  ...
  const defaultUnitName = 'Unidade principal';
  const unitNameOf = (item: any) => String(item?.unit || item?.unitName || item?.unit_name || item?.metadata?.unitName || item?.metadata?.unit || item?.raw_payload?.unitName || item?.raw_payload?.unit || '').trim() || defaultUnitName;
  const availableUnits = Array.from(new Set([...localServices, ...localTeam, ...appointments].map(unitNameOf))).filter(Boolean);
  const unitStats = (availableUnits.length ? availableUnits : [defaultUnitName]).map(unit => ({
    unit,
    services: localServices.filter(item => unitNameOf(item) === unit).length,
    team: localTeam.filter(item => unitNameOf(item) === unit).length,
    appointments: appointments.filter(item => unitNameOf(item) === unit).length
  }));
  useEffect(() => {
    if (unitFilter !== 'all' && !availableUnits.includes(unitFilter)) setUnitFilter('all');
  }, [unitFilter, availableUnits.join('|')]);
  ...
  const filteredAppointments = appointments.filter(item => {
    ...
    const byUnit = unitFilter === 'all' || unitNameOf(item) === unitFilter;
    if (!byStatus || !byUnit) return false;
    if (!query.trim()) return true;
    const haystack = `${item.customer_name || item.name || ''} ${item.customer_phone || item.phone || ''} ${item.customer_email || ''} ${item.service_name || ''} ${item.professional_name || ''} ${unitNameOf(item)} ${item.requested_date || ''} ${item.requested_time || ''} ${item.status || ''}`.toLowerCase();
  ...
  const normalizeServiceForSave = (service: any, index: number) => {
    ...
    const unitName = String(service.unit || service.unitName || service.unit_name || '').trim() || defaultUnitName;
    return {
      ...service,
  ...
  const normalizeTeamForSave = (member: any) => {
    ...
    const unitName = String(member.unit || member.unitName || member.unit_name || '').trim() || defaultUnitName;
    return {
      ...member,
      unit: unitName,
      unitName,
  ...
  const addLocalService = () => setLocalServices(current => [...current, { name: 'Novo serviço', duration: '60', durationMinutes: '60', price: '0', description: 'Descreva o serviço para a página pública.', category: 'Geral', unit: defaultUnitName, unitName: defaultUnitName, active: true, highlight: false, order: current.length + 1, professionalsText: '', internalNotes: '' }]);
  const addLocalTeamMember = () => setLocalTeam(current => [...current, { name: 'Novo profissional', role: 'Profissional', specialty: 'Atendimento', unit: defaultUnitName, unitName: defaultUnitName, whatsapp: businessWhatsapp, email: '', avatarUrl: '', active: true, servicesText: '', availabilityNotes: '' }]);
  ...
  const productionPolishChecks = [
    { key: 'account', label: 'Conta e acesso', description: 'Conta cliente protegida e vinculada ao negócio.', ok: Boolean(account.email), section: 'Configurações' },
    { key: 'plan', label: 'Plano comercial', description: activePlan ? 'Plano liberado para uso do SaaS.' : 'Regularize ou aguarde aprovação do pagamento.', ok: activePlan, section: 'Financeiro' },
    { key: 'public', label: 'Página pública', description: published ? 'Página de apresentação e agendamento disponíveis.' : 'Publique a agenda para liberar os links finais.', ok: Boolean(published), section: 'Página pública' },
    { key: 'services', label: 'Serviços e equipe', description: `${activeLocalServices.length} serviço(s) e ${activeLocalTeam.length} profissional(is) ativos.`, ok: hasFirstService && hasFirstProfessional, section: !hasFirstService ? 'Serviços' : 'Equipe' },
    { key: 'schedule', label: 'Horários', description: hasWorkingSchedule ? 'Disponibilidade pronta para cálculo público.' : 'Configure pelo menos um período de atendimento.', ok: hasWorkingSchedule, section: 'Disponibilidade' },
    { key: 'messages', label: 'Comunicação', description: 'Templates de confirmação, lembrete e pós-atendimento disponíveis.', ok: true, section: 'Mensagens' },
    { key: 'reports', label: 'Relatórios', description: appointments.length ? 'Dados reais alimentando relatórios.' : 'Relatórios prontos para os primeiros agendamentos.', ok: Boolean(published), section: 'Relatórios' }
  ];
  const productionPolishScore = Math.round((productionPolishChecks.filter(item => item.ok).length / productionPolishChecks.length) * 100);
  const nextProductionPolishCheck = productionPolishChecks.find(item => !item.ok) || productionPolishChecks[productionPolishChecks.length - 1];
  ...
  const unitsRank = countBy(reportAppointments, unitNameOf);
  const topUnit = unitsRank[0]?.[0] || defaultUnitName;
  const reportSummary = {
    periodo: periodLabel,
    ...
    unidadeMaisMovimentada: topUnit,
  ...
  const downloadReportCsv = () => {
    const header = ['data','hora','cliente','servico','profissional','unidade','status','valor'];
    const rows = reportAppointments.map(item => [appointmentDateValue(item), item.requested_time || item.time || '', item.customer_name || item.name || '', serviceNameOf(item), professionalNameOf(item), unitNameOf(item), cleanStatus(item.status), String(item.value || item.price || baseServicePrice || 0)]);
  ...
  return <section className="real-agenda-dashboard real-agenda-dashboard-plus premium-motion-scope">
    ...
    <main className="real-agenda-main">
      <header className="real-agenda-topbar dashboard-demo-topbar">
        ...
        <div className="real-agenda-top-actions"><button className="btn secondary compact" onClick={() => loadDashboard(true)} disabled={loading}>{loading ? 'Sincronizando...' : 'Sincronizar'}</button><button className="btn secondary compact" onClick={() => setSection('Unidades')}>Todas as unidades</button><button className="btn secondary compact" onClick={() => setSection('Configurações')}>{businessName}</button><a className="btn secondary compact" href={presentationLink}>Página pública</a></div>
      </header>
      ...
      {section === 'Início' && <>
        ...
        <section className="production-polish-panel">
          <div className="production-polish-copy">
            <Badge tone={productionPolishScore >= 86 ? 'green' : productionPolishScore >= 70 ? 'blue' : 'amber'}>v0.6.4.0</Badge>
            <h3>Prontidão de produção SaaS</h3>
            <p>Leitura compacta dos pontos que deixam a agenda vendável, demonstrável e segura para operar com cliente real.</p>
            <div className="production-polish-meter" aria-label={`Prontidão de produção ${productionPolishScore}%`}><i style={{ width: `${productionPolishScore}%` }} /></div>
            <small>{productionPolishScore}% pronto · próxima revisão: {nextProductionPolishCheck.label}</small>
          </div>
          <div className="production-polish-checks">
            {productionPolishChecks.map(item => <button key={item.key} type="button" className={item.ok ? 'done' : 'pending'} onClick={() => setSection(item.section)}><span>{item.ok ? <CheckCircle2 size={17}/> : <AlertTriangle size={17}/>}</span><b>{item.label}</b><small>{item.description}</small></button>)}
          </div>
          <div className="production-polish-actions">
            <button className="btn primary compact" onClick={() => setSection(nextProductionPolishCheck.section)}>{nextProductionPolishCheck.ok ? 'Ver operação' : 'Corrigir próximo ponto'}</button>
            <button className="btn secondary compact" onClick={() => copy(bookingLink)}>Copiar link público</button>
            <a className="btn secondary compact" href="#/dev">Central Dev</a>
          </div>
        </section>
      ...
      {section === 'Agendamentos' && <section className="booking-management-shell">
        ...
        <article className="real-panel booking-management-panel">
          <div className="booking-toolbar">
            ...
            <label className="unit-filter-select"><Building2 size={16}/><select value={unitFilter} onChange={event => setUnitFilter(event.target.value)}><option value="all">Todas as unidades</option>{availableUnits.map(unit => <option key={unit} value={unit}>{unit}</option>)}</select></label>
          </div>
          {filteredAppointments.length ? <div className="booking-list">{filteredAppointments.map(item => { const statusKey = cleanStatus(item.status); return <article key={item.id || `${item.customer_phone}-${item.requested_date}-${item.requested_time}`} className={`booking-row-card status-${statusKey}`}>
            ...
            <div className="booking-row-main"><strong>{item.customer_name || item.name || 'Cliente'}</strong><span>{item.service_name || primaryService} {item.professional_name ? `• ${item.professional_name}` : ''} <i className="unit-inline-badge">{unitNameOf(item)}</i></span><small>{item.customer_phone || item.phone || 'sem WhatsApp'} {item.customer_email ? `• ${item.customer_email}` : ''}</small>{item.internal_note && <em>Obs. interna: {item.internal_note}</em>}</div>
          ...
          </article>; })}</div> : <div className="real-empty"><CalendarClock/><b>{query || statusFilter !== 'todos' || unitFilter !== 'all' ? 'Nenhum resultado encontrado' : 'Nenhum agendamento recebido ainda'}</b><span>{query || statusFilter !== 'todos' || unitFilter !== 'all' ? 'Tente limpar filtros ou buscar por outro termo.' : 'Compartilhe seu link público para começar a receber horários.'}</span><a className="btn primary compact" href={bookingLink}>Abrir página de agendamento</a></div>}
        </article>
      </section>}
      ...
      {section === 'Serviços' && <section className="team-services-pro-section premium-service-motion">
        ...
        <div className="unit-editor-list"><div><Building2 size={18}/><span><b>Unidade por serviço</b><small>Use para separar catálogo por filial sem alterar a estrutura crítica do banco.</small></span></div>{localServices.length ? localServices.map((service: any, index: number) => <label key={service.id || `service-unit-${index}`}><span>{service.name || `Serviço ${index + 1}`}</span><input value={service.unit || service.unitName || defaultUnitName} onChange={e => updateLocalService(index, { unit: e.target.value, unitName: e.target.value })}/></label>) : <small>Nenhum serviço cadastrado.</small>}</div>
      </section>}
      ...
      {section === 'Equipe' && <section className="team-services-pro-section premium-service-motion">
        ...
        <div className="unit-editor-list"><div><Building2 size={18}/><span><b>Unidade por profissional</b><small>Profissionais podem ficar vinculados a uma unidade sem mudar permissões ou runtime.</small></span></div>{localTeam.length ? localTeam.map((member: any, index: number) => <label key={member.id || `team-unit-${index}`}><span>{member.name || `Profissional ${index + 1}`}</span><input value={member.unit || member.unitName || defaultUnitName} onChange={e => updateLocalTeam(index, { unit: e.target.value, unitName: e.target.value })}/></label>) : <small>Nenhum profissional cadastrado.</small>}</div>
      </section>}
      ...
      {section === 'Unidades' && <section className="unit-foundation-section">
        <article className="real-panel large">
          <div className="panel-heading"><div><Badge tone="green">v0.6.3.9</Badge><h3>Unidades da operação</h3><p>Base multiunidade leve: serviços, profissionais e agendamentos ficam marcados por unidade, mantendo a operação de unidade única sem fricção.</p></div><button className="btn secondary compact" onClick={() => { setUnitFilter('all'); setSection('Agendamentos'); }}>Ver todas</button></div>
          <div className="unit-foundation-grid">{unitStats.map(stat => <button key={stat.unit} type="button" className={unitFilter === stat.unit ? 'active' : ''} onClick={() => { setUnitFilter(stat.unit); setSection('Agendamentos'); }}><Building2 size={22}/><span><b>{stat.unit}</b><small>{stat.services} serviço(s) · {stat.team} profissional(is) · {stat.appointments} agendamento(s)</small></span><Badge tone={stat.unit === defaultUnitName ? 'green' : 'blue'}>{stat.unit === defaultUnitName ? 'Principal' : 'Unidade'}</Badge></button>)}</div>
        </article>
        <article className="real-panel">
          <h3>Como usar</h3>
          <p>Edite a unidade em Serviços e Equipe. Novos agendamentos públicos salvam a unidade no metadata, e o painel filtra sem exigir tabela nova.</p>
          <div className="unit-card-plus"><Building2 size={28}/><div><b>{businessName}</b><span>{businessAddress || defaultUnitName}</span><small>WhatsApp: {businessWhatsapp || 'não informado'}</small></div><Badge tone="green">Ativa</Badge></div>
          <button className="btn secondary full" onClick={() => setSection('Serviços')}>Editar unidades dos serviços</button>
          <button className="btn secondary full" onClick={() => setSection('Equipe')}>Editar unidades da equipe</button>
        </article>
      </section>}
    ...
    {selectedBooking && <div className="detail-drawer-backdrop booking-drawer-backdrop" onClick={() => setSelectedBooking(null)}><aside className="detail-drawer booking-detail-drawer" onClick={event => event.stopPropagation()}>
      ...
      <div className="booking-detail-grid">
        ...
        <span><b>Unidade</b>{unitNameOf(selectedBooking)}</span>
...
function DeveloperConsolePage() {
  ...
  const renderTab = () => {
    ...
    if (activeTab === 'appointments') return renderEntityTable('appointments', rows.appointments, [['Cliente final', item => <><b>{item.customer_name || item.name || 'Cliente'}</b><small>{item.customer_phone || item.phone}</small></>], ['Agenda', item => item.agenda_slug || item.business_name || '—'], ['Serviço', item => item.service_name || item.service || '—'], ['Unidade', item => item.unit_name || item.unit || item.metadata?.unitName || item.metadata?.unit || item.raw_payload?.unitName || 'Unidade principal'], ['Data/hora', item => `${item.requested_date || item.date || '—'} ${item.requested_time || item.start_time || ''}`], ['Status', item => statusBadge(item.status)]], item => quickActions(item, 'appointment'));
