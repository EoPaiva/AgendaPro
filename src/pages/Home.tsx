...
function getAgendaReadiness(agenda?: Partial<AgendaDraft> | null, account?: ClientAccount | null): AgendaReadiness {
  ...
  const activeServices = services.filter(service => String(service?.name || '').trim() && serviceDurationMinutes(service) > 0);
  const activeTeam = team.filter(member => String(member?.name || '').trim());
  ...
  const issues: AgendaReadinessIssue[] = [
    ...
    { key: 'services', title: 'Serviços cadastrados', description: activeServices.length ? `${activeServices.length} serviço(s) pronto(s) para agendamento.` : 'Cadastre pelo menos um serviço com duração válida.', ok: activeServices.length > 0, required: true, actionLabel: 'Cadastrar serviço', step: 2 },
    { key: 'team', title: 'Profissionais', description: activeTeam.length ? `${activeTeam.length} profissional(is) disponível(is).` : 'Cadastre pelo menos um profissional ou responsável.', ok: activeTeam.length > 0, required: true, actionLabel: 'Cadastrar profissional', step: 3 },
...
function PrivateAgendaDashboardPage({ route }: { route: string }) {
  ...
  const addLocalService = () => setLocalServices(current => [...current, { name: 'Novo serviço', duration: '60', price: '0', description: 'Descreva o serviço para a página pública.', category: 'Geral', active: true, highlight: false }]);
  const addLocalTeamMember = () => setLocalTeam(current => [...current, { name: 'Novo profissional', role: 'Profissional', specialty: 'Atendimento', whatsapp: businessWhatsapp, avatarUrl: '', active: true }]);
  ...
  const saveTeamServices = async () => {
    const nextAgenda: AgendaDraft = {
      ...
      services: localServices.filter(item => String(item?.name || '').trim()),
      team: localTeam.filter(item => String(item?.name || '').trim()),
  ...
  const serviceHealthAlerts = [
    !localServices.length ? 'Cadastre pelo menos um serviço para a página pública receber agendamentos.' : '',
    localServices.some(service => !serviceDurationMinutes(service)) ? 'Existe serviço sem duração válida.' : '',
    !localTeam.length ? 'Adicione pelo menos um profissional para organizar o atendimento.' : '',
  ...
  return <section className="real-agenda-dashboard real-agenda-dashboard-plus premium-motion-scope">
    ...
    <main className="real-agenda-main">
      ...
      {section === 'Equipe' && <section className="team-services-pro-section premium-service-motion">
        ...
        <div className="service-editor-grid team-editor-grid">{localTeam.length ? localTeam.map((member: any, index: number) => <article className="service-editor-card team-editor-card" key={member.id || index}><div className="team-avatar-preview">{member.avatarUrl ? <img src={member.avatarUrl} alt={member.name || 'Profissional'} /> : <span>{String(member.name || 'P').slice(0,2).toUpperCase()}</span>}</div><label><span>Nome</span><input value={member.name || ''} onChange={e => updateLocalTeam(index, { name: e.target.value })}/></label><label><span>Função</span><input value={member.role || 'Profissional'} onChange={e => updateLocalTeam(index, { role: e.target.value })}/></label><label><span>Especialidade</span><input value={member.specialty || ''} onChange={e => updateLocalTeam(index, { specialty: e.target.value })}/></label><label><span>WhatsApp</span><input value={member.whatsapp || ''} onChange={e => updateLocalTeam(index, { whatsapp: e.target.value })}/></label><label><span>Foto por URL</span><input value={member.avatarUrl || ''} onChange={e => updateLocalTeam(index, { avatarUrl: e.target.value })}/></label><label><span>Serviços vinculados</span><input value={member.servicesText || localServices.map(s => s.name).filter(Boolean).slice(0,3).join(', ')} onChange={e => updateLocalTeam(index, { servicesText: e.target.value })}/></label><label className="toggle-line compact"><input type="checkbox" checked={member.active !== false} onChange={e => updateLocalTeam(index, { active: e.target.checked })}/><b>Ativo na página pública</b></label><div className="real-button-row"><button className="btn secondary compact" onClick={() => openWhatsApp(member.whatsapp, member.name)}>WhatsApp</button><button className="btn secondary compact danger-soft" onClick={() => removeLocalTeamMember(index)}>Remover</button></div></article>) : <article className="service-editor-card empty"><UsersRound/><h3>Nenhum profissional cadastrado</h3><p>Adicione pelo menos uma pessoa para aparecer na página pública.</p><button className="btn primary compact" onClick={addLocalTeamMember}>Cadastrar profissional</button></article>}</div>
      </section>}
